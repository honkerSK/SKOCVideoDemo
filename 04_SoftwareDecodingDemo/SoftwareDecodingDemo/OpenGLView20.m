...
///当前View的layer
@property(nonatomic, readonly, weak) CALayer *viewLayer;
...
- (void)layoutSubviews
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        glViewport(1, 1, self.viewBounds.size.width*_viewScale - 2, self.viewBounds.size.height*_viewScale - 2);
    });
}
...
{
    [EAGLContext setCurrentContext:_glContext];
    CGSize size = self.viewBounds.size;
...
- (BOOL)createFrameAndRenderBuffer
{
    ...
    if (![_glContext renderbufferStorage:GL_RENDERBUFFER fromDrawable:(CAEAGLLayer *)self.viewLayer])
    {
        NSLog(@"attach渲染缓冲区失败");
    }
...
//    }
    ...
    {
        printf("视频 %ld 帧率:   %ld\n", (long)self.viewTag, (long)_frameRate);
